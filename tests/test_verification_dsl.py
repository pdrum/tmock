...
class TestVerifyCalled:
    def test_called_passes_when_method_was_called(self):
        ...
        define().given(mock.add(any(int), any(int))).returns(0)
        mock.add(1, 2)
        checks().verify(mock.add(1, 2)).called()
    ...
    def test_called_fails_when_called_with_different_args(self):
        ...
        with pytest.raises(TMockVerificationError):
            checks().verify(mock.add(3, 4)).called()
...
class TestVerifyOnce:
    def test_once_passes_when_called_exactly_once(self):
        ...
        define().given(mock.add(any(int), any(int))).returns(0)
        mock.add(1, 2)
        checks().verify(mock.add(1, 2)).once()
...
class TestVerifyTimes:
    def test_times_passes_with_exact_count(self):
        ...
        mock.add(1, 2)
        mock.add(1, 2)
        checks().verify(mock.add(1, 2)).times(3)
...
class TestVerifyNever:
    ...
    def test_never_passes_when_called_with_different_args(self):
        ...
        define().given(mock.add(any(int), any(int))).returns(0)
        mock.add(1, 2)
        checks().verify(mock.add(3, 4)).never()
...
class TestVerifyAtLeast:
    def test_at_least_passes_when_count_equals_minimum(self):
        ...
        mock.add(1, 2)
        mock.add(1, 2)
        checks().verify(mock.add(1, 2)).at_least(2)
...
class TestVerifyAtMost:
    def test_at_most_passes_when_count_equals_maximum(self):
        ...
        mock.add(1, 2)
        mock.add(1, 2)
        checks().verify(mock.add(1, 2)).at_most(2)
    ...
    def test_at_most_zero_same_as_never(self):
        mock = tmock(Calculator)
        checks().verify(mock.add(1, 2)).at_most(0)


class TestVerifyWithKwargs:
    def test_verify_with_kwargs(self):
        mock = tmock(Calculator)
        define().given(mock.greet(any(str), greeting=any(str))).returns("")
        mock.greet("Alice", greeting="Hi")
        checks().verify(mock.greet("Alice", greeting="Hi")).once()
    ...
    def test_verify_kwargs_must_match(self):
        ...
        mock.greet("Alice", greeting="Hi")
        with pytest.raises(TMockVerificationError):
            checks().verify(mock.greet("Alice", greeting="Hello")).called()

    def test_verify_with_default_kwargs(self):
        mock = tmock(Calculator)
        define().given(mock.greet(any(str), greeting=any(str))).returns("")
        mock.greet("Alice")
        checks().verify(mock.greet("Alice")).once()


class TestVerifyWithNoArgs:
    def test_verify_no_arg_method(self):
        mock = tmock(Calculator)
        define().given(mock.no_args()).returns(0)
        mock.no_args()
        checks().verify(mock.no_args()).once()


class TestVerifyWithStubbing:
    def test_verify_works_after_stubbing(self):
        mock = tmock(Calculator)
        define().given(mock.add(1, 2)).returns(100)
    ...
    def test_multiple_verifications_on_same_mock(self):
        ...
        checks().verify(mock.add(1, 2)).once()
        checks().verify(mock.add(3, 4)).once()
...
class TestVerifyErrorMessages:
    ...
    def test_error_message_includes_kwargs(self):
        mock = tmock(Calculator)
        with pytest.raises(TMockVerificationError) as exc_info:
            checks().verify(mock.greet("Alice", greeting="Hi")).called()
...
class TestVerifyEdgeCases:
    ...
    def test_verify_without_mock_call_raises_error(self):
        with pytest.raises(TMockStubbingError) as exc_info:
            checks().verify(None).called()
        assert "checks() was called but no mock method was invoked" in str(exc_info.value)
    ...
    def test_verify_same_method_different_args_independently(self):
        ...
        checks().verify(mock.add(1, 1)).once()
        checks().verify(mock.add(2, 2)).once()
        checks().verify(mock.add(3, 3)).once()
        checks().verify(mock.add(4, 4)).never()


class TestIncompleteVerificationDetection:
    """Tests that incomplete checks().verify() calls are detected and raise errors."""

    def test_incomplete_verify_detected_on_next_mock_call(self):
        mock = tmock(Calculator)
        define().given(mock.add(any(int), any(int))).returns(0)
        mock.add(1, 2)
        checks().verify(mock.add(1, 2))  # Forgot .once(), .called(), etc.
    ...
    def test_incomplete_verify_detected_on_next_checks(self):
        mock = tmock(Calculator)
        define().given(mock.add(any(int), any(int))).returns(0)
        mock.add(1, 2)
        checks().verify(mock.add(1, 2))  # Forgot .once()

        with pytest.raises(TMockVerificationError) as exc_info:
            checks().verify(mock.add(1, 2))  # Next checks() should detect incomplete

        assert "Incomplete verification" in str(exc_info.value)

    def test_incomplete_verify_detected_on_define(self):
        mock = tmock(Calculator)
        define().given(mock.add(any(int), any(int))).returns(0)
        mock.add(1, 2)
        checks().verify(mock.add(1, 2))  # Forgot .once()
        ...
        with pytest.raises(TMockVerificationError) as exc_info:
            define().given(mock.add(1, 2))  # define() should detect incomplete verify
    ...
    def test_complete_verification_allows_subsequent_operations(self):
        ...
        define().given(mock.add(any(int), any(int))).returns(0)
        mock.add(1, 2)
        checks().verify(mock.add(1, 2)).once()  # Complete verification
